...
fn main() {
    ...
    println!("     v1 · v3 (orthogonal) = {:.3}", cosine_similarity(&v1, &v3));
    ...
    for doc in &documents {
        println!("   Created: {} (embedding dim: {})", doc.id, doc.embedding.len());
    }
    ...
    if let Some(doc) = index.get("doc1") {
        println!("   Retrieved doc1: {:?}", doc.content.chars().take(40).collect::<String>());
    ...
    println!("   Removed doc4: {}, index now has {} documents", removed, index.len());
