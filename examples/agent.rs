...
fn main() {
    ...
    println!("   Working memory task: {:?}", memory.get_working("current_task"));
    ...
    println!("   Step {}: State = {}", context.current_step, context.state);
    ...
    println!("   Tool definitions ready for LLM: {}", registry.definitions().len());
    ...
    println!("   Before reset: {} messages, step {}", ctx.memory.message_count(), ctx.current_step);
    ctx.reset();
    println!("   After reset: {} messages, step {}", ctx.memory.message_count(), ctx.current_step);
