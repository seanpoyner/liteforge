...
fn main() {
    ...
    println!("   Total messages (including system): {}", all_messages.len());

    let last_msg = conv.last_message();
    if let Some(msg) = last_msg {
        let content = msg.content.as_deref().unwrap_or("");
        println!("   Last message: \"{}...\"", &content[..content.len().min(40)]);
    }
    ...
    let strategies = [
        (SummarizationStrategy::KeepRecent, "Keep only recent messages, discard old ones"),
        (SummarizationStrategy::Summarize, "Summarize older messages into system context"),
        (SummarizationStrategy::ChunkedSummary, "Create multiple summary levels"),
        (SummarizationStrategy::PreserveSystem, "Always preserve system messages"),
    ];
    ...
    for i in 1..=10 {
        compact_conv.add_user_message(format!("This is user message number {}. It has some content to use tokens.", i));
        compact_conv.add_assistant_message(format!("Response to message {}. Here is a helpful reply with information.", i));
    }

    println!("   Total messages added: {}", compact_conv.total_messages_added());
    ...
    if compact_conv.needs_compaction() {
        if let Some(result) = compact_conv.compact() {
            ...
            if let Some(summary) = &result.summary {
                println!("     Summary preview: \"{}...\"", &summary[..summary.len().min(60)]);
            }
        }
    }
    ...
    println!("     Messages: {}", conv.len());
    println!("     System message preserved: {}", conv.system_message().is_some());
